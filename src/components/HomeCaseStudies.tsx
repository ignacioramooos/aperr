...
const HomeCaseStudies = () => {
  ...
  useEffect(() => {
    supabase
      .from("case_studies")
      .select("id, company_name, company_ticker, company_sector, author_first_name, author_last_initial, summary")
  ...
  return (
    <section className="py-24 md:py-32 border-t border-border">
      <div className="container">
          ...
          {studies.map((s) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                {s.company_ticker && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-secondary text-muted-foreground font-heading">
                    {s.company_ticker}
                  </span>
                )}
              </div>
        ...
        <Link to="/analisis" className="md:hidden flex items-center gap-1 text-sm font-heading font-medium text-muted-foreground hover:text-foreground transition-colors mt-6">
          Ver todos <ArrowRight size={14} />
        </Link>
      </div>
    </section>
