...
const valueProps = [
  {
    icon: Eye,
    title: "Visibilidad",
    desc: "Logo en el sitio, mención en clases presenciales, contenido en redes.",
  },
  {
    icon: Users,
    title: "Acceso a talento",
    desc: "Conocé a los jóvenes más motivados del programa antes que nadie.",
  },
  {
    icon: BarChart3,
    title: "Impacto medible",
    desc: "Recibí un informe mensual con métricas reales del programa.",
  },
];
    ...
    <>
      ...
      <section className="pt-32 md:pt-40 pb-20" style={{ backgroundColor: "#0D1B2A" }}>
        <div className="container max-w-3xl text-center">
          <SectionFade>
            <h1 className="text-3xl md:text-5xl text-white mb-4 font-semibold">
            ...
            <Button
              ...
              size="cta"
              className="gap-2"
              onClick={() => document.getElementById("aliarse")?.scrollIntoView({ behavior: "smooth" })}
            >
          ...
          {partners.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 items-center">
              {partners.map((p) => (
                ...
                >
                  {p.logo_url ? (
                    <img src={p.logo_url} alt={p.name} className="max-h-12 object-contain" />
                  ) : (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground text-lg">
                ¿Tu organización quiere sumarse? Completá el formulario abajo.
              </p>
            </div>
          )}
        </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {collabOptions.map((opt) => (
                    <label key={opt} className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer">
                      <input
                        type="checkbox"
                        checked={form.collaboration_types.includes(opt)}
                        onChange={() => toggleCollab(opt)}
